...
============================================================================*/
void Remote_SendData(uint8_t Add,uint8_t LedData,uint8_t Time,uint8_t Multi,uint8_t Goal,uint8_t Sub_1,uint8_t Sub_2)
{
  uint8_t CheckSum =0;
  ...
  RF_SendPacket(SendBuffer, SEND_LENGTH); //TX 2 times to ensure signal is received by marker  
  SendFlag=0;
}
/*===========================================================================
MAIN FUNCTION
  ...
  while (1)
  {
    switch (Program_Run)
    {
      case P1_Run:
      {
		...
		LEDP1_TOG(); //LED blink for confirmation
		DelayMs(400);
		LEDP1_TOG();
        All_Program(P1_Run);
      ...
      case P2_Run:
      {
		...
		LEDP2_TOG(); //LED blink for confirmation
		DelayMs(400);
		LEDP2_TOG();
		All_Program(P2_Run);
        Mode=0;
      ...
      case P3_Run:
      {
		...
		LEDP3_TOG(); //LED blink for confirmation
		DelayMs(400);
		LEDP3_TOG();
		All_Program(P3_Run);
        Mode=0;
      ...
      case P4_Run:
      {
		...
		LEDP4_TOG(); //LED blink for confirmation
		DelayMs(400);
		LEDP4_TOG();
		All_Program(P4_Run);
        Mode=0;
      ...
      case P5_Run:
      {
		...
		LEDP5_TOG(); //LED blink for confirmation
		DelayMs(400);
		LEDP5_TOG();
		All_Program(P5_Run);
        Mode=0;
	  ...
	  case P4_C3G_Run:
	  {
		  USART_Send("P43G Running\r\n", 13);
		  LEDP4_TOG(); //LED blink for confirmation
		  LED3G_TOG();
		  DelayMs(400);
		  LEDP4_TOG();
		  LED3G_TOG();
		  All_Program(P4_C3G_Run);
		  Mode = 0;
		  Program_Run = 0;
		  break;
	  }
	  case P5_C3G_Run:
	  {
		  USART_Send("P53G Running\r\n", 13);
		  LEDP5_TOG(); //LED blink for confirmation
		  LED3G_TOG();
		  DelayMs(400);
		  LEDP5_TOG();
		  LED3G_TOG();
		  All_Program(P5_C3G_Run);
		  Mode = 0;
		  Program_Run = 0;
		  break;
	  }
	  ...
	  case P4_C4G_Run:
	  {
		  USART_Send("P44G Running\r\n", 13);
		  LEDP4_TOG(); //LED blink for confirmation
		  LED4G_TOG();
		  DelayMs(400);
		  LEDP4_TOG();
		  LED4G_TOG();
		  All_Program(P4_C4G_Run);
		  Mode = 0;
		  Program_Run = 0;
		  break;
	  }
	  case P5_C4G_Run:
	  {
		  USART_Send("P54G Running\r\n", 13);
		  LEDP5_TOG(); //LED blink for confirmation
		  LED4G_TOG();
		  DelayMs(400);
		  LEDP5_TOG();
		  LED4G_TOG();
		  All_Program(P5_C4G_Run);
		  Mode = 0;
		  Program_Run = 0;
		  break;
	  }
